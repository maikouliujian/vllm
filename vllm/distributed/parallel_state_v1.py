...
def init_distributed_environment(
    ...
    # todo 添加分卡打印日志！！！！！！
    # setup_vllm_logging(rank)
    setup_vllm_logging_copytruncate(rank)
    if local_rank == -1:
...
def setup_vllm_logging_copytruncate(rank):
    """在Python中实现copytruncate"""
    import os
    import sys
    import threading
    import time
    ...
    debug_split_log = os.environ.get("VLLM_DEBUG_SPLIT_LOG", "0") == "1"
    if not debug_split_log:
        return

    try:
        log_dir = os.environ.get("VLLM_LOG_DIR", "./vllm_logs")
        max_size_mb = int(os.environ.get("LOG_SIZE_MB", 100))
        keep_files = int(os.environ.get("KEEP_FILES", 5))
        os.makedirs(log_dir, exist_ok=True)

        # 固定文件名（不轮转文件名，只轮转内容）
        fixed_log_file = os.path.join(
            log_dir,
            f"vllm_rank{rank:03d}_{rank}.log"
        )

        # 初始打开（追加模式）
        fd = os.open(fixed_log_file, os.O_WRONLY | os.O_CREAT | os.O_APPEND, 0o644)
        ...
        print(f"=== vLLM copytruncate日志系统 ===")
        print(f"=== Rank: {rank}")
        print(f"=== 固定文件: {fixed_log_file} (inode不变)")
        print("=" * 50)
        ...
        def copytruncate():
            """执行copytruncate：复制内容到新文件，然后截断原文件"""
            print(f"[轮转] 开始copytruncate...")

            # 1. 获取当前inode信息
            stat_info = os.stat(fixed_log_file)
            ...
            archive_file = f"{fixed_log_file}.{timestamp}.gz"
            ...
            try:
                ...
                # 复制并压缩
                with open(fixed_log_file, 'rb') as f_in:
                ...
                print(f"[轮转] 已归档到: {os.path.basename(archive_file)}")

            except Exception as e:
                print(f"[轮转] 归档失败: {e}")
                return

            # 4. 截断原文件（关键！保持相同inode）
            try:
                # 先刷新所有缓冲区
                sys.stdout.flush()
                sys.stderr.flush()
                ...
                # 验证inode未改变
                new_stat = os.stat(fixed_log_file)
                if new_stat.st_ino == original_inode:
                    print(f"[轮转] 文件已截断，inode保持不变: {original_inode}")
                else:
                    print(f"[轮转] 警告: inode已改变！ {original_inode} -> {new_stat.st_ino}")
            ...
            try:
                import glob
                pattern = f"{os.path.basename(fixed_log_file)}.*.gz"
                ...
                for old_archive in archive_files[keep_files:]:
                    try:
                        os.remove(old_archive)
                        print(f"[清理] 删除: {os.path.basename(old_archive)}")
                    except:
                        pass
        ...
        def monitor_and_rotate():
            ...
            while True:
                try:
                    if os.path.exists(fixed_log_file):
                        size = os.path.getsize(fixed_log_file)
                        if size >= max_size:
                            print(f"[监控] 文件大小: {size // 1024 // 1024}MB, 触发轮转")
                            copytruncate()
        ...
        monitor_thread = threading.Thread(target=monitor_and_rotate, daemon=True)
        monitor_thread.start()

        print(f"[初始化] copytruncate监控已启动")

        return {
            'fixed_log_file': fixed_log_file,
            'fd': fd,
            'original_stdout': original_stdout,
            'original_stderr': original_stderr,
            'copytruncate_func': copytruncate
        }

    except Exception as e:
        import traceback
        os.write(2, f"vLLM日志初始化失败: {e}\n".encode())
        traceback.print_exc()


def setup_vllm_logging(rank):
    """设置vLLM分布式日志"""
    import os
    # 检查环境变量
    debug_split_log = os.environ.get("VLLM_DEBUG_SPLIT_LOG", "0") == "1"
    ...
    try:
        ...
        import logging
        from datetime import datetime
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")  # 这里需要 datetime
        log_file = os.path.join(
            log_dir,
            f"vllm_rank{rank:03d}_{rank}_{timestamp}.log"
        )
        ...
        print(f"=== vLLM 日志系统初始化 ===")
        print(f"===Rank: {rank}")
        print("=" * 50)

    except Exception as e:
        print(f"vLLM日志初始化失败: {e}", file=sys.stderr)
